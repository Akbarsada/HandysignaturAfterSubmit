...
$pdfFileResult =   get_option( 'has_gv_form3pdfWP_Download_Gesamtpfad' ) . '/' . $filename . '_signed.pdf';
$certificate =    get_option( 'has_gv_form3ZertifikatsDateiOrdnerSpeicherPfad' ) . '/' . $filename . '_signer.cer';
...
$tempId = get_option( 'has_gv_form3SiegelTemplatesAuswahlID' );
...
$client_finish = file_get_contents( 'wp-content/plugins/HandysignaturAfterSubmitReload/src/inc/signature/connect3/finish.php');
$client_file_finish = 'finish.php';
file_put_contents( $client_file_finish , $client_finish);

$weiterleitung_serialized = serialize(get_option( 'has_gv_form3Erfolg' ));
