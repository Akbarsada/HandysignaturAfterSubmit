...
$errorUrl = get_option( 'has_gv_form5Error' );
$handySignaturParameter = "";
$location = "";
$reason = "";
$pdfFile =  $copy_to_dir . $filename;
$pdfFileResult =   get_option( 'has_gv_form5pdfWP_Download_Gesamtpfad' ) . '/' . $filename . '_signed.pdf';
$certificate =    get_option( 'has_gv_form5ZertifikatsDateiOrdnerSpeicherPfad' ) . '/' . $filename . '_signer.cer';
...
$tempId = get_option( 'has_gv_form5SiegelTemplatesAuswahlID' );
...
$client_finish = file_get_contents( 'wp-content/plugins/HandysignaturAfterSubmitReload/src/inc/signature/connect5/finish.php');
$client_file_finish = 'finish.php';
file_put_contents( $client_file_finish , $client_finish);

$weiterleitung_serialized = serialize(get_option( 'has_gv_form5Erfolg' ));
